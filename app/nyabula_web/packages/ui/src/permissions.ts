/* Permission display metadata shared by panel/console.
 * Chinese names come from the protocol mapping table
 * (Shared/protocol/nyalink.md, "权限中文名"); permission ids never change.
 * `icon` is a Material Icons (Apache-2.0, by Google) 24x24 filled path so no
 * icon font is required at runtime. */
...
/* Material Icons path shorthands reused across several permissions. */
const ICON_SETTINGS =
...
const ICON_WIFI =
  'M1 9l2 2c4.97-4.97 13.03-4.97 18 0l2-2C16.93 2.93 7.08 2.93 1 9zm8 8 3 3 3-3c-1.65-1.66-4.34-1.66-6 0zm-4-4 2 2c2.76-2.76 7.24-2.76 10 0l2-2C15.14 9.14 8.87 9.14 5 13z';
const ICON_LOCK =
  'M18 8h-1V6c0-2.76-2.24-5-5-5S7 3.24 7 6v2H6c-1.1 0-2 .9-2 2v10c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V10c0-1.1-.9-2-2-2zm-6 9c-1.1 0-2-.9-2-2s.9-2 2-2 2 .9 2 2-.9 2-2 2zm3.1-9H8.9V6c0-1.71 1.39-3.1 3.1-3.1 1.71 0 3.1 1.39 3.1 3.1v2z';

/** Permission id -> display metadata. Keys mirror the protocol table 1:1. */
export const PERMISSION_META: Record<string, PermissionMeta> = {
  'servo.control': { zh: '舵机控制', icon: ICON_SETTINGS },
  'storage.private': {
    zh: '私有存储',
    icon: 'M10 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V8c0-1.1-.9-2-2-2h-8l-2-2z',
  },
  'storage.shared.read': { zh: '共享存储读取', icon: ICON_STORAGE },
  'storage.shared.write': { zh: '共享存储写入', icon: ICON_STORAGE },
  'gpio.read': { zh: 'GPIO 读取', icon: ICON_MEMORY },
  'gpio.write': { zh: 'GPIO 控制', icon: ICON_MEMORY },
  'network.request': { zh: '网络请求', icon: ICON_PUBLIC },
  'network.raw_socket': { zh: '原始网络套接字', icon: ICON_ETHERNET },
  'bluetooth.scan': { zh: '蓝牙扫描', icon: ICON_BLUETOOTH },
  'bluetooth.connect': { zh: '蓝牙连接', icon: ICON_BLUETOOTH },
  'wifi.status': { zh: 'WiFi 状态', icon: ICON_WIFI },
  'wifi.control': { zh: 'WiFi 控制', icon: ICON_WIFI },
  'audio.play': {
    zh: '音频播放',
  ...
  'camera.capture': {
    zh: '摄像头',
    icon: 'M9 2 7.17 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2h-3.17L15 2H9zm3 15c-2.76 0-5-2.24-5-5s2.24-5 5-5 5 2.24 5 5-2.24 5-5 5zm0-2c1.66 0 3-1.34 3-3s-1.34-3-3-3-3 1.34-3 3 1.34 3 3 3z',
  },
  'location.read': {
    zh: '位置信息',
...
/** Fallback icon (lock) for permission ids not in the table. */
export const PERMISSION_FALLBACK_ICON = ICON_LOCK;

/** Chinese display name for a permission id (falls back to the id itself). */
export function permissionZh(id: string): string {
  return PERMISSION_META[id]?.zh ?? id;
}

/** Icon path for a permission id (falls back to a lock icon). */
export function permissionIcon(id: string): string {
  return PERMISSION_META[id]?.icon ?? PERMISSION_FALLBACK_ICON;
}
