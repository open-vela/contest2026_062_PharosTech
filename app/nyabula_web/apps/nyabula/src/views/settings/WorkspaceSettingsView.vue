...
<style scoped>
/* One gap for the section grid and the account/client list below it. */
.settings-hub { --tile-gap: 8px; max-width: 1000px; margin: 0 auto; padding: 20px; display: grid; gap: 28px; }
h2 { font: 600 20px var(--font-title); margin: 0 0 8px; }
.settings-links { display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: var(--tile-gap); margin-top: 12px; }
.settings-list { display: grid; gap: var(--tile-gap); margin-top: 12px; }
.list-tile { min-height: 52px; }
/* Let the subtitle wrap on narrow screens instead of being cut off. */
.list-tile .tile-sub { white-space: normal; }
