...
#define EMMC_RESPERR_INTS       EMMC_MKERR(EMMC_EINT_CMDTIMEOUT | \
                                           EMMC_EINT_CMDCRC | \
                                           EMMC_EINT_CMDEND | \
                                           EMMC_EINT_CMDIDX)
#define EMMC_DATAERR_INTS       EMMC_MKERR(EMMC_EINT_DATTIMEOUT | \
                                           EMMC_EINT_DATCRC | \
                                           EMMC_EINT_DATEND)
...
static int  rk3576_emmc_recvsetup(struct sdio_dev_s *dev, uint8_t *buffer,
                                  size_t nbytes);
static int  rk3576_emmc_sendsetup(struct sdio_dev_s *dev,
                                 ...
                                 uint32_t freq)
{
  ...
  clk = EMMC_CLKCTRL_INTLEN |
        (uint16_t)(div << EMMC_CLKCTRL_SDCLKFREQ_SHIFT);
  ...
  rk3576_emmc_putreg16(priv, RK3576_EMMC_CLKCTRL,
                       clk | EMMC_CLKCTRL_SDCLKEN);
}
  ...
  /* --- Command done --- */
  ...
  else if ((nint & EMMC_NINT_CMDDONE) != 0)
    {
      if ((priv->waitevents & (SDIOWAIT_CMDDONE | SDIOWAIT_RESPONSEDONE))
          != 0)
        {
          rk3576_emmc_endwait(priv, priv->waitevents &
...
static void rk3576_emmc_reset(struct sdio_dev_s *dev)
{
  ...
  basemhz = (cap0 >> 8) & 0xff;
  priv->baseclk = basemhz != 0 ? basemhz * 1000000 :
                                 RK3576_EMMC_BASECLK_DEF;
  ...
  for (i = 0; i < RK3576_EMMC_SPIN; i++)
    {
      ...
      uint32_t busy = EMMC_PRESENT_CMDINHIBIT |
                      (needdat ? EMMC_PRESENT_DATINHIBIT : 0);
  ...
  for (i = 0; i < RK3576_EMMC_SPIN; i++)
    {
      ...
      if (crccheck &&
          (eint & (EMMC_EINT_CMDCRC | EMMC_EINT_CMDEND | EMMC_EINT_CMDIDX))
          != 0)
        {
