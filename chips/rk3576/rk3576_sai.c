...
static uint32_t rk3576_sai_getreg(struct rk3576_sai_s *priv,
                                  unsigned int offset);
static void     rk3576_sai_putreg(struct rk3576_sai_s *priv,
                                  unsigned int offset, uint32_t value);
static void     rk3576_sai_modifyreg(struct rk3576_sai_s *priv,
                                  unsigned int offset, uint32_t clrbits,
                                  uint32_t setbits);
static void     rk3576_sai_clearlogic(struct rk3576_sai_s *priv,
                                  uint32_t clrbit);
...
static void     rk3576_sai_dmacallback(struct rk3576_dmach_s *ch,
                                  int result, void *arg);
...
static int      rk3576_sai_txchannels(struct i2s_dev_s *dev,
                                  uint8_t channels);
static uint32_t rk3576_sai_txsamplerate(struct i2s_dev_s *dev,
                                  uint32_t rate);
static uint32_t rk3576_sai_txdatawidth(struct i2s_dev_s *dev, int bits);
static int      rk3576_sai_send(struct i2s_dev_s *dev,
                                  struct ap_buffer_s *apb,
                                  i2s_callback_t callback, void *arg,
                                  uint32_t timeout);
static int      rk3576_sai_rxchannels(struct i2s_dev_s *dev,
                                  uint8_t channels);
static uint32_t rk3576_sai_rxsamplerate(struct i2s_dev_s *dev,
                                  uint32_t rate);
static uint32_t rk3576_sai_rxdatawidth(struct i2s_dev_s *dev, int bits);
static int      rk3576_sai_receive(struct i2s_dev_s *dev,
                                  struct ap_buffer_s *apb,
                                  i2s_callback_t callback, void *arg,
                                  uint32_t timeout);
static uint32_t rk3576_sai_getmclk(struct i2s_dev_s *dev);
static uint32_t rk3576_sai_setmclk(struct i2s_dev_s *dev,
                                  uint32_t frequency);
static int      rk3576_sai_ioctl(struct i2s_dev_s *dev, int cmd,
                                  unsigned long arg);
