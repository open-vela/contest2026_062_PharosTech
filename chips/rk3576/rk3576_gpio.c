...
{
  RK3576_GPIO0_ADDR,    /* GPIO0: 0x27320000 */
  RK3576_GPIO1_ADDR,    /* GPIO1: 0x2AE10000 */
  RK3576_GPIO2_ADDR,    /* GPIO2: 0x2AE20000 */
  RK3576_GPIO3_ADDR,    /* GPIO3: 0x2AE30000 */
  RK3576_GPIO4_ADDR,    /* GPIO4: 0x2AE40000 */
};
...
{
  /* GPIO0: offsets 0x0000(A), 0x0008(B), 0x2004(C), 0x200C(D) */
  [0] = {
  ...
  /* GPIO1: offsets 0x4020(A), 0x4028(B), 0x4030(C), 0x4038(D) */
  [1] = {
    { .offset = 0x4020 },
    { .offset = 0x4028 },
    { .offset = 0x4030 },
  ...
  /* GPIO2: offsets 0x4040(A), 0x4048(B), 0x4050(C), 0x4058(D) */
  [2] = {
    { .offset = 0x4040 },
    { .offset = 0x4048 },
    { .offset = 0x4050 },
  ...
  /* GPIO3: offsets 0x4060(A), 0x4068(B), 0x4070(C), 0x4078(D) */
  [3] = {
    { .offset = 0x4060 },
    { .offset = 0x4068 },
    { .offset = 0x4070 },
  ...
  /* GPIO4: offsets 0x4080(A), 0x4088(B), 0xA390(C), 0xB398(D) */
  [4] = {
    { .offset = 0x4080 },
    { .offset = 0x4088 },
    { .offset = 0xA390 },
 ...
 *
 * All IOMUX/pull/drive/schmitt register offsets are relative to PMU0_IOC,
 * which is the first module in the IOC cluster covering GPIO0-GPIO4.
 *
 * IOC cluster layout (verified against RK3576 TRM):
...
static inline void rk3576_gpio_dirout(unsigned int port, unsigned int pin)
{
  ...
  regval = getreg32(RK3576_GPIO_SWPORTA_DDR(port));
  regval |= RK3576_GPIO_PIN_BIT(pin);
  putreg32(regval, RK3576_GPIO_SWPORTA_DDR(port));
}
                             ...
                             unsigned int pin, unsigned int pull)
{
  ...
  if (port == 0 && pin < 12)
    ...
    {
      reg_offset = 0x2028 - 0x4;  /* GPIO0_BH, already adjusted for pin offset */
    }
  else if (port == 1)
    {
      reg_offset = 0x6110;
  ...
  switch (mode)
    {
      ...
      case GPIO_OUTPUT:
        {
          _warn("GPIO config: OUTPUT port=%u pin=%u ioc=0x%08lx gpio_base=0x%08lx\n",
                port, pin,
                (unsigned long)g_ioc_base,
                (unsigned long)g_gpio_base[port]);

          /* Set IOMUX to GPIO function (AF=0) before configuring direction.
           * Bootloader may have left IOMUX in a non-GPIO state.
           */

          rk3576_iomux_set(g_ioc_base, port, pin, 0);
  ...
  if ((pinset & GPIO_EXTI) != 0)
    {
      ...
      regval = getreg32(RK3576_GPIO_INTTYPE_LEVEL(port));
      if ((pinset & GPIO_INTTYPE_MASK) == GPIO_INT_EDGE)
        {
          regval |= RK3576_GPIO_PIN_BIT(pin);
        }
      else
      ...
      putreg32(regval, RK3576_GPIO_INTTYPE_LEVEL(port));

      /* Set interrupt polarity */

      regval = getreg32(RK3576_GPIO_INT_POLARITY(port));
      if ((pinset & GPIO_INTPOL_MASK) == GPIO_INT_HIGH_RISING)
        {
          regval |= RK3576_GPIO_PIN_BIT(pin);
        }
      else
      ...
      putreg32(regval, RK3576_GPIO_INT_POLARITY(port));

      /* Clear any pending interrupt */

      putreg32(RK3576_GPIO_PIN_BIT(pin), RK3576_GPIO_PORTA_EOI(port));

      /* Unmask and enable the interrupt */

      regval = getreg32(RK3576_GPIO_INTMASK(port));
      regval &= ~RK3576_GPIO_PIN_BIT(pin);  /* Clear mask to enable */
      putreg32(regval, RK3576_GPIO_INTMASK(port));

      regval = getreg32(RK3576_GPIO_INTEN(port));
      regval |= RK3576_GPIO_PIN_BIT(pin);
      putreg32(regval, RK3576_GPIO_INTEN(port));
...
void rk3576_gpio_write(gpio_pinset_t pinset, bool value)
{
  ...
  regval = getreg32(RK3576_GPIO_SWPORTA_DR(port));
  ...
  putreg32(regval, RK3576_GPIO_SWPORTA_DR(port));
}
...
static int g_gpio_input_count   = 0;
static int g_gpio_output_count  = 0;
static int g_gpio_int_count     = 0;
...
static struct rk3576_gpio_dev_s
  *g_gpio_inputs[RK3576_GPIO_NPORTS * RK3576_GPIO_NPINS];
static struct rk3576_gpio_dev_s
  *g_gpio_outputs[RK3576_GPIO_NPORTS * RK3576_GPIO_NPINS];
...
int rk3576_gpio_register_input(gpio_pinset_t pinset)
{
  ...
  ret = gpio_pin_register(&dev->gpio, g_gpio_input_count);
  if (ret < 0)
    {
      kmm_free(dev);
      return ret;
  ...
  rk3576_config_gpio(pinset);
  g_gpio_inputs[g_gpio_input_count] = dev;
  g_gpio_input_count++;
...
int rk3576_gpio_register_output(gpio_pinset_t pinset)
{
  ...
  ret = gpio_pin_register(&dev->gpio,
                          g_gpio_input_count + g_gpio_output_count);
  if (ret < 0)
    {
      kmm_free(dev);
      return ret;
  ...
  rk3576_config_gpio(pinset);
  g_gpio_outputs[g_gpio_output_count] = dev;
  g_gpio_output_count++;
...
int rk3576_gpio_register_interrupt(gpio_pinset_t pinset)
{
  ...
  ret = gpio_pin_register(&dev->dev.gpio,
                          g_gpio_input_count + g_gpio_output_count +
                          g_gpio_int_count);
  if (ret < 0)
    {
