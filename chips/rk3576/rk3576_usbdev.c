...
/****************************************************************************
 * USB device (peripheral) controller driver for the RK3576 USB OTG0 port,
 * a Synopsys DesignWare USB3 (DWC3) dual-role controller at 0x23000000.
 *
 * Scope of this driver:
 *   - Device mode only.  The port capability is forced to peripheral, so
 *     the Type-C role controller (husb311) is not involved.
...
#include "rk3576_usbdev.h"

#ifdef CONFIG_RK3576_USBDEV
...
static uint32_t rk3576_usbdev_getreg(uint32_t offset);
static void rk3576_usbdev_putreg(uint32_t val, uint32_t offset);
static int rk3576_usbdev_epcmd(uint8_t phyep, uint32_t cmd, uint32_t p0,
...
static int rk3576_usbdev_starttrb(struct rk3576_ep_s *privep, void *buf,
                                  uint32_t len, uint32_t trbtype);
static void rk3576_usbdev_reqcomplete(struct rk3576_ep_s *privep,
                                      int16_t result);
static void rk3576_usbdev_epnext(struct rk3576_ep_s *privep);
...
/****************************************************************************
 * Name: rk3576_usbdev_getreg / rk3576_usbdev_putreg
 ****************************************************************************/

static uint32_t rk3576_usbdev_getreg(uint32_t offset)
{
  return getreg32(RK3576_OTG_BASE + offset);
}

static void rk3576_usbdev_putreg(uint32_t val, uint32_t offset)
{
  putreg32(val, RK3576_OTG_BASE + offset);
}

/****************************************************************************
 * Name: rk3576_usbdev_epcmd
 *
 * Description:
                               ...
                               uint32_t p1, uint32_t p2, uint32_t *rsc)
{
  ...
  rk3576_usbdev_putreg(p0, DWC3_DEPCMDPAR0(phyep));
  rk3576_usbdev_putreg(p1, DWC3_DEPCMDPAR1(phyep));
  rk3576_usbdev_putreg(p2, DWC3_DEPCMDPAR2(phyep));
  rk3576_usbdev_putreg(cmd | DEPCMD_CMDACT, DWC3_DEPCMD(phyep));

  do
    {
      reg = rk3576_usbdev_getreg(DWC3_DEPCMD(phyep));
  ...
  if (retries <= 0)
    {
      uerr("epcmd %lu on phyep %u timeout (GSTS=%08lx DSTS=%08lx "
           ...
           (unsigned long)rk3576_usbdev_getreg(DWC3_GSTS),
           (unsigned long)rk3576_usbdev_getreg(DWC3_DSTS),
           (unsigned long)rk3576_usbdev_getreg(DWC3_GCTL));
      return -ETIMEDOUT;
    }
...
/****************************************************************************
 * Name: rk3576_usbdev_ep0out_arm_setup
 *
 * Description:
...
/****************************************************************************
 * Name: rk3576_usbdev_reqcomplete
 *
 * Description:
...
static void rk3576_usbdev_epnext(struct rk3576_ep_s *privep)
{
  ...
  if (privreq->req.buf == NULL)
    {
      rk3576_usbdev_reqcomplete(privep, -EINVAL);
      return;
    }
  ...
  if (in)
    {
      ...
      rk3576_usbdev_starttrb(privep, privreq->req.buf, privreq->req.len,
                             TRB_TYPE_NORMAL);
    }
  else
    {
...
static void rk3576_usbdev_ep0setup(struct rk3576_usbdev_s *priv)
{
  ...
  if (priv->eps[1].busy)
    {
      ...
      while (sq_peek(&priv->eps[1].reqq) != NULL)
        {
          rk3576_usbdev_reqcomplete(&priv->eps[1], -ESHUTDOWN);
        }
    }
  ...
  if ((ctrl->type & USB_REQ_TYPE_MASK) == USB_REQ_TYPE_STANDARD &&
    ...
    {
      uint32_t dcfg = rk3576_usbdev_getreg(DWC3_DCFG);
  ...
  if (ret < 0)
    {
      ...
      rk3576_usbdev_epcmd(0, DEPCMD_SETSTALL, 0, 0, 0, NULL);
      rk3576_usbdev_ep0out_arm_setup(priv);
      return;
    }
      ...
      switch (priv->ep0state)
        {
          ...
          case EP0_DATA_IN:
            priv->ep0state = EP0_STATUS_WAIT;
            rk3576_usbdev_reqcomplete(&priv->eps[1], OK);
            break;
      ...
      if ((status & 3) == DEPEVT_NRDY_CTL_STATUS &&
      ...
      else if ((status & 3) == DEPEVT_NRDY_CTL_DATA)
        {
          ...
          if (priv->ep0pend && priv->ep0state == EP0_DATA_IN)
            {
              priv->ep0pend = false;
              rk3576_usbdev_starttrb(&priv->eps[1], priv->ep0buf,
...
static void rk3576_usbdev_epevent(struct rk3576_usbdev_s *priv, uint32_t evt)
{
  ...
  if (phyep < 2)
    {
      rk3576_usbdev_ep0event(priv, evt);
      return;
    }
      ...
      if (privreq != NULL)
        {
          ...
          rk3576_usbdev_reqcomplete(privep, OK);
        }
      ...
      case DEVT_USBRST:
        {
          ...
          dcfg = rk3576_usbdev_getreg(DWC3_DCFG);
          dcfg &= ~DCFG_DEVADDR_MASK;
          rk3576_usbdev_putreg(dcfg, DWC3_DCFG);
          priv->devaddr = 0;
...
static int rk3576_usbdev_interrupt(int irq, void *context, void *arg)
{
  struct rk3576_usbdev_s *priv = &g_usbdev;
  uint32_t count;
  uint32_t evt;

  count = rk3576_usbdev_getreg(DWC3_GEVNTCOUNT) & 0xffffu;
  ...
  while (count != 0)
    {
      ...
      if (EVT_IS_DEVT(evt))
        {
          rk3576_usbdev_devevent(priv, evt);
        }
      else
        {
          rk3576_usbdev_epevent(priv, evt);
        }

      /* Tell the controller we consumed one event */

      rk3576_usbdev_putreg(4, DWC3_GEVNTCOUNT);
...
static int rk3576_usbdev_coreinit(struct rk3576_usbdev_s *priv)
{
  ...
  reg = rk3576_usbdev_getreg(DWC3_GSNPSID);
  uinfo("GSNPSID=%08lx\n", (unsigned long)reg);
  ...
  reg = rk3576_usbdev_getreg(DWC3_GCTL);
  reg |= GCTL_CORESOFTRESET;
  rk3576_usbdev_putreg(reg, DWC3_GCTL);

  reg = rk3576_usbdev_getreg(DWC3_GUSB2PHYCFG);
  rk3576_usbdev_putreg(reg | GUSB2PHYCFG_PHYSOFTRST, DWC3_GUSB2PHYCFG);
  reg = rk3576_usbdev_getreg(DWC3_GUSB3PIPECTL);
  rk3576_usbdev_putreg(reg | GUSB3PIPECTL_PHYSOFTRST, DWC3_GUSB3PIPECTL);
  up_mdelay(1);
  ...
  rk3576_usbdev_putreg(DCTL_CSFTRST, DWC3_DCTL);
  retries = 1000;
  do
    {
      reg = rk3576_usbdev_getreg(DWC3_DCTL);
  ...
  rk3576_usbdev_putreg(reg, DWC3_GUSB2PHYCFG);
  ...
  rk3576_usbdev_putreg(reg, DWC3_GUSB3PIPECTL);
  ...
  rk3576_usbdev_putreg((uint32_t)((uintptr_t)priv->evtbuf & 0xffffffffu),
                       DWC3_GEVNTADRLO);
  rk3576_usbdev_putreg((uint32_t)((uintptr_t)priv->evtbuf >> 32),
                       DWC3_GEVNTADRHI);
  rk3576_usbdev_putreg(RK3576_EVTBUF_SIZE, DWC3_GEVNTSIZ);
  rk3576_usbdev_putreg(0, DWC3_GEVNTCOUNT);

  /* Device speed: high speed (USB 2.0) */

  reg = rk3576_usbdev_getreg(DWC3_DCFG);
  reg &= ~(DCFG_DEVSPD_MASK | DCFG_DEVADDR_MASK);
  reg |= DCFG_DEVSPD_HS;
  rk3576_usbdev_putreg(reg, DWC3_DCFG);

  /* Start endpoint configuration: DEPSTARTCFG with resource index 0
   * assigns transfer resources for ep0 OUT/IN.
   */

  ret = rk3576_usbdev_epcmd(0, DEPCMD_DEPSTARTCFG, 0, 0, 0, NULL);
  if (ret < 0)
    {
      return ret;
    }

  ret = rk3576_usbdev_epconfig(&priv->eps[0], USB_EP_ATTR_XFER_CONTROL,
  ...
  ret = rk3576_usbdev_epconfig(&priv->eps[1], USB_EP_ATTR_XFER_CONTROL,
                               RK3576_EP0_MAXPACKET, false);
  if (ret < 0)
    {
      return ret;
  ...
  rk3576_usbdev_putreg(3, DWC3_DALEPENA);
  rk3576_usbdev_ep0out_arm_setup(priv);
                               ...
                               const struct usb_epdesc_s *desc, bool last)
{
  ...
  ret = rk3576_usbdev_epconfig(privep, eptype, maxpacket, false);
  if (ret < 0)
    {
      return ret;
    }
  ...
  reg = rk3576_usbdev_getreg(DWC3_DALEPENA);
  reg |= (1u << privep->phyep);
  rk3576_usbdev_putreg(reg, DWC3_DALEPENA);
...
static int rk3576_dev_pullup(struct usbdev_s *dev, bool enable)
{
  ...
  rk3576_usbdev_putreg(reg, DWC3_DCTL);
  uinfo("pullup %d\n", enable);
  return OK;
}
...
int rk3576_usbdev_initialize(void)
{
  ...
  irq_attach(RK3576_IRQ_OTG, rk3576_usbdev_interrupt, NULL);
  up_enable_irq(RK3576_IRQ_OTG);

  return rk3576_usbdev_coreinit(priv);
}
